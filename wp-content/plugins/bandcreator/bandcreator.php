        ...
        <h3>Registered Artists</h3>
                ...
                <?php
                        foreach($results as $result){
                                ...
                                echo('<div id="userartistactions"><a class="artistactions" href="' . get_option('siteurl') . '/add-music?aid=' . $result->id . '">Music Manager</a><a class="artistactions" href="' . get_option('siteurl') . '/update-profile">Update Profile</a><a class="artistactions" href="' . get_option('siteurl') . '/remove-artist">Delete Artist</a></div>');
                                echo('</div>');
                                echo("</li>");
                        }
                }?>
...
function gnrb_edit_band() {
	...
	$user_id = $_POST["user_id"];
        if($_POST["import_band_fromdb"]!=-1){
                ...
                //echo("I need to update".$import_userid);
                $success=true;
	...
	foreach( $results as $f ) {
		$field_name = "field$f->id";
                if($f->field_key=="band_name")
                        $wpdb->query("update $gnrb_table_artist set name='$f->field_value' where id=$artistid");
                
                if($f->field_key=="band_email")
                        $wpdb->query("update $gnrb_table_artist set email='$f->field_value' where id=$artistid");
...
function gnrb_get_bands_list() {
	...
	$sql = "select id,name from $gnrb_table_artist";
        $results = $wpdb->get_results($sql);
...
function gnrb_get_band_navigator($current){
	...
	foreach(range('A','Z') as $letter){
                if($letter==$current)
                        print("<li><a class=\"currentartistpage\" href=\"http://test.radioverve.com/new/artists?start=$letter\">$letter</a></li>");
                else
                        print("<li><a href=\"http://test.radioverve.com/new/artists?start=$letter\">$letter</a></li>");
        }
...
//        $sql = "select user_id,user_nicename from wp_gnrb_artist, wp_users where wp_gnrb_artist.user_id=wp_users.id and wp_users.user_nicename like '$start%' order by wp_users.user_nicename";
        ...
        $sql = "select id,name from wp_gnrb_artist where name like '$start%' order by name";
        //echo($sql);
...
function gnrvp_get_artist_featured_info($user_id,$band_name){
	...
	$band_icon = get_option("gnrb_band_pics_path") . $band_icon;
        $band_url="http://test.radioverve.com/new/artist/".sanitize_title_with_dashes($band_name);
