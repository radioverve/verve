<?php
/***********************Function for Slideshow Module START*******************/
/*
    * function get_slideshow_data
    * @param $count
*/

function get_slideshow_data($count=0) {
	global $wpdb;
	$i = 0;
        $row = $wpdb->get_results( "select wtr.object_id from $wpdb->term_taxonomy wtt, $wpdb->term_relationships wtr where wtt.term_id=(select term_id from $wpdb->terms where name='Featured') and wtt.term_taxonomy_id = wtr.term_taxonomy_id order by wtr.object_id desc limit " . $count . ";" );
        foreach($row as $val) {
                $tmp_array[$i] = $val->object_id;
                $title[$i] = $wpdb->get_row("select post_title from $wpdb->posts where ID=" . $val->object_id . ";");
                $image[$i] = $wpdb->get_row("select meta_value from $wpdb->postmeta where meta_key='FEATURED_IMAGE' and post_id=" . $val->object_id . " limit 1;");
                $i++;
        }
        
        //Merge the array before returning data
        $return_array = array('title'=>$title,'image'=> $image);
        
        return $return_array;
}
/***********************Function for Slideshow Module END*******************/
